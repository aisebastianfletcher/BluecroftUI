...
export const LTVChart: React.FC<ChartsProps> = ({ metrics, propertyValue, loanAmount }) => {
  ...
  return (
    <div className="h-72 w-full bg-white/90 backdrop-blur-md p-6 rounded-3xl shadow-soft border border-white/60">
      <h3 className="text-sm font-bold text-slate-600 mb-4 uppercase tracking-wider">Capital Structure</h3>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart layout="vertical" data={data} margin={{ top: 5, right: 30, left: 20, bottom: 25 }}>
...
export const RiskGauge: React.FC<{ score: number }> = ({ score }) => {
  ...
  return (
    <div className="h-72 w-full bg-white/90 backdrop-blur-md p-6 rounded-3xl shadow-soft border border-white/60 flex flex-col items-center justify-center relative">
      <h3 className="text-sm font-bold text-slate-600 absolute top-6 left-6 uppercase tracking-wider">AI Risk Score</h3>
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie data={data} cx="50%" cy="50%" startAngle={180} endAngle={0} innerRadius={70} outerRadius={95} paddingAngle={0} dataKey="value" stroke="none">
      ...
      <div className="absolute bottom-6 flex flex-col items-center">
        <span className="text-4xl font-extrabold" style={{ color: getColor(score) }}>{score}</span>
      </div>
    </div>
  );
};
